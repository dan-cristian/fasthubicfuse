...
static int cfs_open(const char* path, struct fuse_file_info* info)
{
   ...
   if (*temp_dir)
   {
      ...
      else
      {
         ...
         debugf(DBG_LEVEL_EXT, "cfs_open: opening custom temp for write");
         ...
         {
            debugf(DBG_LEVEL_NORM,
                   KRED"exit 1: cfs_open cannot open custom temp=[%s] err=%d:%s", file_path_safe,
                   errsv, strerror(errsv));
            return -ENOENT;
         }
      ...
      if (de)
      {
         ...
         if (offset == 0)
         {
            ...
            pthread_create(&de->upload_buf.thread, NULL,
                           (void*)cloudfs_object_upload_progressive, path_copy);
